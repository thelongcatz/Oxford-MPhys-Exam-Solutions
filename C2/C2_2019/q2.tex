...
\begin{parts}
	...
	\begin{align*}
		...
		&= 4\left(\frac{1}{2} - \frac{L}{R_1} - \frac{L}{R_2} \left(1-\frac{L}{R_1}\right) \right) \\
		&= 4\left[\left(1-\frac{L}{R_1}\right) \left(1-\frac{L}{R_2}\right) - \frac{1}{2}\right]
	\end{align*}
	
	Plugging this into \eqref{eqn:low-loss-cond} then gives:
	\begin{gather}
		-\frac{1}{2} < \left(1-\frac{L}{R_1}\right) \left(1-\frac{L}{R_2}\right) - \frac{1}{2} < \frac{1}{2} \notag \\
		0 < \underbracket{\left(1-\frac{L}{R_1}\right)}_{g_1} \underbracket{\left(1-\frac{L}{R_2}\right)}_{g_2} < 1 \label{eqn:low-loss-cond-sph}
	\end{gather}
	...
	\begin{gather*}
		...
		\Rightarrow z_0^\prime - iz_R^\prime = -\left( \frac{L}{2} \right)^2 \times \frac{1}{-iz_R} \\
		\Rightarrow z_0^\prime = 0 \textnormal{\hspace{1em}and\hspace{1em}} z_R z_R^\prime = \left( \frac{L}{2} \right)^2
	\end{gather*}
	
	Next we note that $\exp\left(\dfrac{ik\rho^2}{2q}\right) = \exp\left[\dfrac{ik\rho^2}{2}\left(\dfrac{i}{z_R}\right)\right]=\exp\left[-\dfrac{k\rho^2}{2z_R}\right]$ and thus the spot size $w \propto z_R$ ($\rho^2 = x^2 + y^2$ are the in-plane coordinates).
	
	Also we have $z_R^\prime = \dfrac{1}{z_R} \left(\dfrac{L}{2}\right)^2$ and that $z_R^{\prime\prime} = z_R$ by the low-loss condition.
