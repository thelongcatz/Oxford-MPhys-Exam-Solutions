...
\begin{parts}
	...
	\begin{align*}
		...
		\Rightarrow \mu_0 \mathbf{J} &= -\frac{\partial}{\partial x}\left(B_0 \frac{\cosh\left(x/\lambda\right)}{\cosh\left(t/\lambda\right)}\right) \hat{\mathbf{y}} \\
		\mathbf{J} &= -\frac{B_0}{\mu_0 \lambda} \frac{\sinh\left(x/\lambda\right)}{\cosh\left(t/\lambda\right)} \hat{\mathbf{y}}
	\end{align*}
	...
	For $\lambda \gg t$, we see that $B(x) \rightarrow B_0 \left[1 + \mathcal{O}\left(x^2/\lambda^2\right)\right]$ so we have the B field virtually penetrating the sample.
	...
	Also for type-II SC, upper field corresponds to when $\left(2\pi\xi^2\right)B_c \Phi_0$ where $\xi$ is coherence length:
	\begin{gather*}
		\left(2\pi\xi^2\right)B_c \Phi_0 = \SI{100}{\tesla} \\
