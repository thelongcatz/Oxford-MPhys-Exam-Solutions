...
\begin{parts}
	...
	I'd say that it tests more on Ampere's Law\footnote{As an anecdote, the author bombed this part by creating a new universe in which Ampere's Law took the form of $\int\mathbf{B}\cdot\mathrm{d}\mathbf{A}$\dots} than SC!
	\begin{subparts}
		...
		So $\alpha = \left(\dfrac{\mu_0}{2\pi r_1}\right)^2$, $\beta = \left(\dfrac{\mu_0 N}{L}\right)^2$.
		...
		\begin{gather*}
			...
			\Rightarrow \beta = \frac{1}{I_2^2} \left[\left( B_\textnormal{c}^\textnormal{Ta}(T_1) \right)^2 - \alpha I_1^2\right] = \left(\mu_0 n_\textnormal{c}\right)^2 \\
			\Rightarrow n_\textnormal{c} = \frac{1}{\mu_0 I_2} \sqrt{\left( B_\textnormal{c}^\textnormal{Ta}(T_1) \right)^2 - \alpha I_1^2} = \SI{1.02e5}{\per\metre} = \SI{102}{\per\milli\metre}
		\end{gather*}
	...
	\begin{subparts}
		\subpart Recall from BCS theory that $\lambda = g(E_\textnormal{F}) \left|g_\textnormal{eff}\right|^2$ is the phonon-electron coupling parameter -- it encodes the interaction between phonon and electron near the Fermi surface and has been shown to be attractive by Cooper.
		...
		\begin{equation}
			\lambda \int_{0}^{\hbar\omega_\textnormal{D}} \mathrm{d}\epsilon \frac{1}{\sqrt{\epsilon^2 + \Delta^2}} = 1
